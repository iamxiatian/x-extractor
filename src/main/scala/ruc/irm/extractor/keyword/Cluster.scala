...
object Cluster {
  ...
  val clustering = (text: String, maxK: Int) => {
    val words = acceptWords(text).filter(w => w.length > 1 && word2vec.contains(w)) //可以接受的指定词性的词语，并且在word2vec中出现过
    val data = words.map(word2vec.vectorInDouble).toArray //转换为二维数组，每一行为一个词向量

    val clusters: KMeans = xmeans(data, maxK) // kmeans(data, k, runs = 20)
    ...
    //选取距离聚类中心点最近的词语作为聚类名称返回
    for (c <- 0 until k) //yield words(findNearDoc(0, c, (0, Double.MaxValue))._1)
      yield {
        val doc = findNearDoc(0, c, (0, Double.MaxValue))
        println(s"find near doc for cluster $c, best doc is $doc")
        words(doc._1)
      }
  }
  ...
  /**
    * 测试第id篇文档的聚类结果
    */
  def testArticle(id:Int, maxK:Int) = {
    val article = Articles.getArticle(id)
    clustering(article("title") + article("content"), maxK)
  }
